    ...
    return (
      <div className='Google-map-wrapper'>
        <ButtonGroup className='Map-button-group' style={{marginBottom:'20px'}}>
          ...
          {
            placeTypes.map((place_type, i)=>{ //map関数。必ず配列をreturnできるから。
              return <MenuItem data-type={place_type.id} key={i} onClick={this.nearbySearch}>{place_type.name}</MenuItem> //key={i}はおまじない。
            })
          }
          </DropdownButton>
        </ButtonGroup>
